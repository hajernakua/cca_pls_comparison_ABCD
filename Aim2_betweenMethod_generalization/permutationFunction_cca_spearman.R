set.seed(12348)
CCA_permutation_spearman <- function(X, Y, numboot) {
  ...
  for(i in 1:numboot) {
    ...
    omega_matrix <- t(solve(chol(Sxx))) %*% Sxy %*% solve(chol(Syy))
    
    ##decomposition
    decomposition <- svd(omega_matrix)
...
CCA_permutation_spearman_output <- foreach(i=1:100) %dopar% {
  CCA_permutation_spearman_output <- CCA_permutation_spearman(Baseline_CBCL_CT_SiteFam_Data_NoSib_cbclScores, Baseline_CBCL_CT_SiteFam_Data_NoSib_CT, 100) #calling a function
}
