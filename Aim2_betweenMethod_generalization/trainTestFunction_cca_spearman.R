set.seed(1267)
CCA_traintest_spearman <- function(X, Y, numboot){
  ...
  for(i in 1:numboot) {
    ...
    ##getting omega matrix of the training subset
    Sxx_train <- cor(training_X1[[i]], method = "spearman")
    Syy_train <- cor(training_Y1[[i]], method = "spearman")
    Rxy_train <- cor(training_Y1[[i]], training_X1[[i]], method = "spearman")
    omega_train <- t(solve(chol(Syy_train))) %*% Rxy_train %*% solve(chol(Sxx_train))
    
    #decomposition 
    svd_omega <- svd(omega_train)
    
    ##getting omega matrix of the testing subset
    Sxx_test <- cor(testing_X2[[i]], method = "spearman")
    Syy_test <- cor(testing_Y2[[i]], method = "spearman")
    Rxy_test <- cor(testing_Y2[[i]], testing_X2[[i]], method = "spearman")
    
    omega_test <- t(solve(chol(Syy_test))) %*% Rxy_test %*% solve(chol(Sxx_test))
    
    ##arranging the equation to solve for the singular values of the test set
    sing_val_test_prediction <- t(svd_omega$u) %*% omega_test %*% svd_omega$v
...
CCA_testTrain_spearman_output <- foreach(i=1:100) %dopar% {
  CCA_testTrain_spearman_output <- CCA_traintest_spearman(Baseline_CBCL_CT_SiteFam_Data_NoSib_cbclScores, Baseline_CBCL_CT_SiteFam_Data_NoSib_CT, 100, 9191, 0.5) #calling a function
}

save(CCA_testTrain_spearman_output, file = "CCA_testTrain_spearman_output.RData")
